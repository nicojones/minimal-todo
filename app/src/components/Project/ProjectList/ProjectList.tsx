...
export function ProjectList ({ projectId, changeToProject }: ProjectListAttrs) {
  ...
  useEffect(() => {
    const unsubscribeProjects = projectService.getListOfProjects((_projects: IProject[]) => {
      ...
      <ul className="projects-list flex-column">
        {
          ...
          >
            <button className="ib left left-align w-100" onClick={ () => setProject({ id: p.url } as IProject) }>
