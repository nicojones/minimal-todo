...
export function Project ({ changeToProject }: ProjectAttrs) {
  ...
  const [projectName, setProjectName] = useState(project.name || text.project.noName);
  ...
  useEffect(() => {
    ...
    const unsubscribeTasks = taskService.getTasksForProject(project.id, sort, (tasks: ITask[]) => {
