...
export const ProjectDispatch = React.createContext<Dispatch<SetStateAction<Partial<IProject>>>>({} as unknown as any);
...
export function TodoApp () {
  ...
  const [showSidebar, setShowSidebar] = useState(!(window as any).isSmallScreen);
  ...
  useEffect(() => {
    if (!urlParams.current.projectId) { // There's some URL?
      setComponent(<NoProject className="" setShowSidebar={ setShowSidebar } addText={text.project.noSelected}
